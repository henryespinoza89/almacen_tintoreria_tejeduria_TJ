...
<script type="text/javascript">
  $(function(){
    $("#marca").append('<option value="" selected="selected">:: SELECCIONE ::</option>');
        ...
        buttons: {
          'Eliminar': function () {
            var parent = $(this).data('parent');
                ...
                $(this).dialog('close');
                setTimeout('window.location.href="<?php echo base_url(); ?>comercial/modelomaquina"', 200);
        },
            'Cancelar': function () {
                  $(this).dialog('close');
            }
  ...
  // Editar Máquina
  function editar_modelo_maquina(id_modelo_maquina){
        var urlMaq = '<?php echo base_url();?>comercial/editarmodelomaquina/'+id_modelo_maquina;
        //alert(urlMaq);
        $("#mdlEditarNombreMaquina").load(urlMaq).dialog({
          ...
          buttons: {
            ...
            var editmodelomaq = $('#editmodelomaq').val(); editmarcamaq = $('#editmarcamaq').val(); 
            if(editmodelomaq == '' || editmarcamaq == ''){
              ...
              var dataString = 'editmodelomaq='+editmodelomaq+'&editmarcamaq='+editmarcamaq+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>';
              $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>comercial/actualizarmodelomaquina/"+id_modelo_maquina,
                data: dataString,
                success: function(msg){
                  if(msg == 1){
                    $("#finregistro").html('!El Modelo de la Máquina ha sido actualizada con éxito!.').dialog({
                      modal: true,position: 'center',width: 400,height: 125,resizable: false, title: 'Fin de Registro',
                      buttons: { Ok: function(){
                        window.location.href="<?php echo base_url();?>comercial/modelomaquina";
                      }}
                    });
                  }else{
...
<body>
  <div id="contenedor">
    <div id="tituloCont">Registrar Nuevo Modelo de Máquinas</div>
    <div id="formFiltro">
        <?php echo form_open(base_url()."comercial/registrarmodelomaquina", 'id="registrar"') ?>
          <table width="979" border="0" cellspacing="0" cellpadding="0" style="margin-top: 4px;">
            <tr>
              <td width="240">Seleccione la Marca de la Máquina:</td>
              <?php
                  $existe = count($listamarca);
                  if($existe <= 0){ ?>
              ...
              <?php    
                  ...
                  {
                ?>
                    <td width="200"><?php echo form_dropdown('marca',$listamarca,'','id="marca" style="width:158px;"');?></td>
              <?php }?>
            </tr>
            <tr>
              <td width="208">Ingrese el Modelo de la Marca:</td>
        ...
        <?php echo form_close() ?>
        <div id="tituloCont" style="border-bottom-style:none;">Lista de Modelos asociado a una Marca</div>
        <!--Iniciar listar-->
        <?php 
          $existe = count($modelomaquinas);
          if($existe <= 0){
            echo 'No existen Modelos de Máquinas registradas en el Sistema.';
          }
          else
          {
        ?>
        <table border="0" cellspacing="0" cellpadding="0" id="listaNombreMaquinas">
          <thead>
            <tr class="tituloTable">
              <td sort="idproducto" width="150" height="25">ID Modelo Máquina</td>
          ...
          <?php 
            $i=1;
            foreach($modelomaquinas as $listamodelomaquinas){ ?>  
          <tr class="contentTable">
            <!--<td><?php //echo str_pad($listamodelomaquinas->id_modelo_maquina, 5, 0, STR_PAD_LEFT); ?></td>-->
            <td height="27"><?php echo str_pad($i,4,0, STR_PAD_LEFT);?></td>
            <td><?php echo $listamodelomaquinas->no_marca; ?></td>
            <td><?php echo $listamodelomaquinas->no_modelo; ?></td>
            <td width="20" align="center"><img class="editar_modelo_maquina" src="<?php echo base_url();?>assets/img/edit.png" width="20" height="20" title="Editar Nombre de Máquina" onClick="editar_modelo_maquina(<?php echo $listamodelomaquinas->id_modelo_maquina; ?>)" /></td>
            <td width="20" align="center">
              <a href="" class="eliminar_registro" id="elim_<?php echo $listamodelomaquinas->id_modelo_maquina; ?>">
              <img src="<?php echo base_url();?>assets/img/trash.png" width="20" height="20" title="Eliminar Modelo de Máquina"/></a>
            </td>
          </tr>
          <?php
            $i++;
  ...
  <div style="display:none">
    <div id="direccionelim"><?php echo site_url('comercial/eliminarmodelomaquina');?></div>
  </div>
  <div id="dialog-confirm" style="display: none;" title="Eliminar Modelo de Máquina">
    <p>
      <span class="ui-icon ui-icon-alert" style="float: left; margin: 0 7px 20px 0;"></span>
      ¿Está seguro que quiere eliminar este Modelo de Máquina?<br /><strong>¡Esta acción no se puede revertir!</strong>
    </p>
  </div>